...
public class SonarOutlineLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(@NotNull PoseStack poseStack, MultiBufferSource buffer, int packedLight,
        ...
        if (ClientState.renderMode == RenderMode.ECHO_LOCALIZATION) {
            r = 1.0f; g = 1.0f; b = 0.2f;
        }

        RenderType outline = RenderType.outline(this.getTextureLocation(livingEntity));
    ...
    public static class ClientState {
        ...
        public static RenderMode renderMode = RenderMode.SONAR;

        private static int lastTicks = 0;

        public static void setTicksToRenderEntities(int ticks, RenderMode mode) {
