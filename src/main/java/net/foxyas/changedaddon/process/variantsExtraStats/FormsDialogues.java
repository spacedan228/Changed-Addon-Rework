...
public class FormsDialogues {
    ...
    private static void normalWorldHandle(Player player, TransfurVariantInstance<?> PlayerVariant) {
        if (PlayerVariant.getParent().is(ChangedAddonTransfurVariants.EXPERIMENT_10_BOSS.get())) {
            ...
            if (randomSelector <= 4) { 
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp10.death.text1"), false);
            } else {
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp10.death.text2"), false);
            }
        } else if (PlayerVariant.getParent().is(ChangedAddonTransfurVariants.EXPERIMENT_009_BOSS.get())) {
            int randomSelector = player.getLevel().getRandom().nextInt(100);
            if (randomSelector <= 4) { 
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp9.death.text1"), false);
            } else {
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp9.death.text2"), false);
            }
            ...
            int randomSelector = player.getLevel().getRandom().nextInt(100);
            if (randomSelector <= 3) { 
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp1.death.text1"), false);
            } else if (randomSelector <= 6) {
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp1.death.text2"), false);
            } else if (randomSelector <= 9) {
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp1.death.text3"), false);
            } else if (randomSelector <= 12) {
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp1.death.text4", player.getDisplayName().getString()), false);
            }
            ...
            int randomSelector = player.getLevel().getRandom().nextInt(100);
            if (randomSelector <= 4) { 
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp2.death.text1"), false);
            } else if (randomSelector <= 7) { 
                player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp2.death.text2"), false);
            }
        }
    }
            ...
            if (changedVariantEvent.livingEntity instanceof Player player) {
                if (YTNames.contains(player.getUUID())) {
                    if (player.getLevel().isClientSide()) {
                        player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp9.transfur.text.secret"), false);
                    }
                }
            }
        }
        if (changedVariantEvent.newVariant.is(ChangedAddonTransfurVariants.EXP1_MALE.get()) || changedVariantEvent.newVariant.is(ChangedAddonTransfurVariants.EXP1_FEMALE.get())) {
            if (changedVariantEvent.livingEntity instanceof Player player) {
                if (FBNames.contains(player.getUUID())) {
                    if (player.getLevel().isClientSide()) {
                        player.displayClientMessage(new TranslatableComponent("changed_addon.entity_dialogues.exp1.transfur.text.secret"), false);
                    }
                }
            }
        }
